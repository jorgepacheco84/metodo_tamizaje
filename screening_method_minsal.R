...
m_uci_31 <- glm(cbind(uci_vac, uci_no_vac) ~ offset(o), family=binomial,data=ejemplo_screening_sem[["sem_31"]])
m_uci_32 <- glm(cbind(uci_vac, uci_no_vac) ~ offset(o), family=binomial,data=ejemplo_screening_sem[["sem_32"]])
m_def_31 <- glm(cbind(def_vac, def_no_vac) ~ offset(o), family=binomial,data=ejemplo_screening_sem[["sem_31"]])
m_def_32 <- glm(cbind(def_vac, def_no_vac) ~ offset(o), family=binomial,data=ejemplo_screening_sem[["sem_32"]])
...
efectividad_semana <- data.frame(semana = rep(c("31", "32"), each = 2),
                                 Desenlace = rep(c("UCI", "Fallecer"), times = 2),
                                 VE = c(VE_uci_31, VE_def_31, VE_uci_32, VE_def_32),
                                 lb = c(VE_uci_31_ic[1], VE_def_31_ic[1], VE_uci_32_ic[1], VE_def_32_ic[1]),
                                 ub = c(VE_uci_31_ic[2], VE_def_31_ic[2], VE_uci_32_ic[2], VE_def_32_ic[2]))

ggplot(efectividad_semana, aes(x = semana, y=VE)) + 
  geom_errorbar(aes(ymin=lb, ymax=ub), width=.1) +
  ggtitle ("VE estimada por método de tamizaje para cada semana") +
